...
export default async function (app: FastifyInstance) {
  app.get('/v1/exports/events.csv', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const header = 'type,id,studentId,terminalId,schoolId,ts,confidence,method,mealType\n';
    const rows = [
      ...attendance.map((r: any) => `attendance,${r.id},${r.student_id},${r.terminal_id},${r.school_id},${r.ts},${r.confidence},,`),
      ...meals.map((r: any) => `meal,${r.id},${r.student_id},${r.terminal_id},${r.school_id},${r.ts},,${r.method},${r.meal_type ?? 'lunch'}`),
    ];
